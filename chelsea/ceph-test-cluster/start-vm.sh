...
# Extra disks for Ceph
sudo curl -X PUT --unix-socket "${API_SOCKET}" \
    --data "{
        \"drive_id\": \"osd1\",
        \"path_on_host\": \"./osd1.ext4\",
        \"is_root_device\": false,
        \"is_read_only\": false
    }" \
        "http://localhost/drives/osd1"

sudo curl -X PUT --unix-socket "${API_SOCKET}" \
    --data "{
        \"drive_id\": \"osd2\",
        \"path_on_host\": \"./osd2.ext4\",
        \"is_root_device\": false,
        \"is_read_only\": false
    }" \
    "http://localhost/drives/osd2"

sudo curl -X PUT --unix-socket "${API_SOCKET}" \
    --data "{
        \"drive_id\": \"osd3\",
        \"path_on_host\": \"./osd3.ext4\",
        \"is_root_device\": false,
        \"is_read_only\": false
    }" \
    "http://localhost/drives/osd3"
