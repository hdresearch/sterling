...
# The rootfs doesn't contain SSH keys to connect to the VM
# So create one and patch it in the rootfs
...
truncate -s 20G ubuntu-$ubuntu_version.ext4
sudo mkfs.ext4 -d squashfs-root -F ubuntu-$ubuntu_version.ext4

# Create disks for Ceph
truncate -s 10G osd1.ext4
truncate -s 10G osd2.ext4
truncate -s 10G osd3.ext4
